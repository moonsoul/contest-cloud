...
<%--
	...
	var curExpandNode = null;
		function beforeExpand(treeId, treeNode) {
			...
			var zTree = $.fn.zTree.getZTreeObj("treeDemoPai19");
...
////////////////////////////////////////////////
	...
	function onClick19(e, treeId, treeNode) {
		...
		var cityObj = $("#citySelPai19").attr("value", v);
	...
	function hideMenu() {
		$("#menuContentPai19").fadeOut("fast");
		$("body").unbind("mousedown", onBodyDown);
	}
	function onBodyDown(event) {
		if (!(event.target.id == "menuBtnPai19" || event.target.id == "menuContentPai19" || $(
				event.target).parents("#menuContentPai19").length > 0)) {
			hideMenu();
		}
	}
<%--
	$(document).ready(function() {
		$.fn.zTree.init($("#treeDemoPai19"), setting, zNodes19);
	});--%>
</script>

<form id="pagerForm" method="post"  action="${ctx}/UserManageAction.do?method=getUserListPaiMing">
...
<div class="pageHeader">
		...
		<form rel="pagerForm"
		action="${ctx}/UserManageAction.do?method=getUserListPaiMing"
...
<div id="menuContentPai19" class="menuContent" style="display: none; position: absolute;background-color:#EEF3F4; border-radius:6px;">
	<ul id="treeDemoPai19" class="ztree" style="margin-top: 0; width: 175px;overflow-x:scroll;"></ul>
</div>
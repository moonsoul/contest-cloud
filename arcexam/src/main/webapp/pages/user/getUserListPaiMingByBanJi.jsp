...
<script type="text/javascript">
	...
	var curExpandNode = null;
		function beforeExpand(treeId, treeNode) {
			...
			var zTree = $.fn.zTree.getZTreeObj("treeBanji19");
...
////////////////////////////////////////////////
	...
	function onClick19(e, treeId, treeNode) {
		...
		var cityObj = $("#citySelBanJi19").attr("value", v);
		var parentid = $("#searchgroupid").attr("value", t);
		//alert($("#searchgroupid").val());

	}

	function showMenuBanJi19(a) {
		var cityObj = $("#citySelBanJi19");
	...
	function hideMenu() {
		$("#menuContentBanji19").fadeOut("fast");
		$("body").unbind("mousedown", onBodyDown);
	}
	function onBodyDown(event) {
		if (!(event.target.id == "menuBtnBanJi19" || event.target.id == "menuContentBanji19" || $(
				event.target).parents("#menuContentBanji19").length > 0)) {
			hideMenu();
		}
	}

	$(document).ready(function() {
		$.fn.zTree.init($("#treeBanji19"), setting, zNodes19);
	});
</script>

<form id="pagerForm" method="post"  action="${ctx}/UserManageAction.do?method=getUserListPaiMingByBanJi">
...
<div class="pageHeader">
		...
		<form rel="pagerForm"
		action="${ctx}/UserManageAction.do?method=getUserListPaiMingByBanJi"
		...
		<div class="searchBar">
			<ul class="searchContent">
				...
				<input size="25" name="searchgroupid" id="searchgroupid" type="hidden" value="${userManageForm.map.searchgroupid}"/> 
				<input readonly="readonly" name="groupname" id="citySelBanJi19" type="text"  value="${userManageForm.map.groupname}"/> 
				<a id="menuBtnBanJi19" class="menuBtn" href="#" onclick="showMenuBanJi19(this); return false;">选择</a>
				</li>
				<li>
					<label>
						用户名：
...
<div id="menuContentBanji19" class="menuContent" style="display: none; position: absolute;background-color:#EEF3F4; border-radius:6px;">
	<ul id="treeBanji19" class="ztree" style="margin-top: 0; width: 175px;overflow-x:scroll;"></ul>
</div>